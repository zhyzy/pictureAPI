...
const defaultAdminPassword = process.env.ADMIN_PASSWORD || crypto.randomBytes(18).toString('base64url');
...
function createTables() {
  ...
  if (adminCheck.length === 0 || adminCheck[0].values.length === 0) {
    if (!process.env.ADMIN_PASSWORD) {
      console.warn('未配置 ADMIN_PASSWORD，已为初始管理员生成随机密码。请通过环境变量显式设置管理员密码。');
    }
