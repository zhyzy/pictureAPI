...
export default function CategoriesPage() {
  ...
  const [formData, setFormData] = useState({ name: '', slug: '', description: '', type: 'image', storage_provider: 'inherit' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', slug: '', description: '', type: 'image', storage_provider: 'inherit' });
      fetchCategories();
    } catch (err: any) {
          ...
          <table className="min-w-full">
            <thead className="bg-[var(--color-bg-subtle)]">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-text-tertiary)] uppercase">类型</th>
                ...
                <tr key={category.id} className="hover:bg-[var(--color-bg-subtle)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      {category.type === 'video' ? '🎬 视频' : '🖼️ 图片'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-[var(--color-text-secondary)]">
                    {category.storage_provider === 'cos'
                      ? '腾讯云 COS'
                      : category.storage_provider === 'qiniu'
                        ? '七牛云'
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-[var(--color-bg-elevated)] rounded-lg shadow-lift p-6 w-full max-w-md border border-[var(--color-border)]">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="label">分类类型 *</label>
                  <select
                    value={formData.type}
                    onChange={(e) => setFormData({ ...formData, type: e.target.value })}
                    className="input"
                  >
                    <option value="image">🖼️ 图片分类（/api/v1/random/）</option>
                    <option value="video">🎬 视频分类（/api/v1/video/）</option>
                  </select>
                  <p className="mt-1 text-xs text-[var(--color-text-tertiary)]">
                    {editingCategory && editingCategory.type !== formData.type ? '注意：更改类型后，已上传的内容不会被移动' : '决定上传内容和调用的 API 接口'}
                  </p>
                </div>
                <div>
