          ...
          <div className="card">
            ...
            <div className="mb-4">
              ...
              {(categories.length > 0 ? categories : []).map((cat) => {
                const base = (siteSettings.site_url || (typeof window !== 'undefined' ? window.location.origin : '')).replace(/\/+$/, '');
                const endpoint = cat.type === 'video' ? 'video' : 'random';
                const fullUrl = `${base}/api/v1/${endpoint}/${cat.slug}?key=${user?.api_key}`;
                return (
