...
async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      const { name, slug, description, storage_provider, type } = req.body;

      if (!name || !slug) {
        return res.status(400).json({ error: '分类名称name和slug必填' });
      }

      const categoryType = type === 'video' ? 'video' : 'image';
      const now = new Date().toISOString();

      try {
        db.run(
          'INSERT INTO categories (name, slug, description, type, storage_provider, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [name, slug, description || null, categoryType, storage_provider || 'inherit', now, now]
        );
    ...
    if (req.method === 'PUT') {
      const { id, name, slug, description, storage_provider, type } = req.body;

      if (!id) {
        return res.status(400).json({ error: 'id必填' });
      }

      const categoryType = type === 'video' ? 'video' : type === 'image' ? 'image' : null;
      const now = new Date().toISOString();

      // 保留原 type（未显式传 type 时）
      const existing = db.get('SELECT type FROM categories WHERE id = ?', [id]);
      const finalType = categoryType || existing?.type || 'image';

      try {
        db.run(
          'UPDATE categories SET name = ?, slug = ?, description = ?, type = ?, storage_provider = ?, updated_at = ? WHERE id = ?',
          [name, slug, description || null, finalType, storage_provider || 'inherit', now, id]
        );
