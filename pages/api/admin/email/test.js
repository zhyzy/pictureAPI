...
async function handler(req, res) {
  ...
  try {
    ...
    console.log('[Email Test] SMTP 配置:', {
      host: cfg.host, port: cfg.port, secure: cfg.secure, user: cfg.user, from: cfg.from, hasPass: !!cfg.pass,
    });
    ...
    const result = await sendEmail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #c75b39;">樱道 API - 邮件测试</h2>
    ...
    if (result.success) {
      ...
      return res.status(500).json({
        ...
        smtp: { host: cfg.host, port: cfg.port, secure: cfg.secure, user: cfg.user },
      });
    }
  } catch (error) {
