...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (categoryObj.type === 'video') {
      return res.status(400).json({ error: '该分类是视频分类，请调用 /api/v1/video/' + category });
    }
    ...
    // 随机取一张图片（避免把整个分类加载进内存；仅取图片，视频走视频接口）
    const randomImage = db.get(
      "SELECT * FROM images WHERE category_id = ? AND media_type = 'image' ORDER BY RANDOM() LIMIT 1",
      [categoryObj.id]
    );
    if (!randomImage) {
    ...
    // type=img/image/redirect：302 跳转到图片本身，可直接放进 <img src> 当随机图床用
    const returnType = (req.query.type || 'json').toString().toLowerCase();
    if (['img', 'image', 'redirect'].includes(returnType)) {
